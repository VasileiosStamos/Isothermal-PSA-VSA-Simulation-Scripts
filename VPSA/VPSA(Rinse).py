...
u_feed = float(config.get("u_feed_rinse", 0.00))
...
# --- PDE SOLVER (Ergun + ideal gas) ---
# P(z) = sum(C)*R*T; inlet u prescribed, outlet zero-gradient.
@njit(cache=True)
def calc_rhs(t, y, N, P_low, P_mid, u_feed, eps, rho_s, dz, MW, mu, y_feed, k_ldf, q_s, b_toth, R, T, a_ergun_arr, b_ergun_arr):
    ...
    # Pass 1: cell-centred quantities
    P_cell   = np.empty(N)
    rho_cell = np.empty(N)
    yf0_arr  = np.empty(N)
    yf1_arr  = np.empty(N)
    ...
    for j in range(N):
        ...
        # Toth isotherm with local pressure
        P_0 = yf0 * P_local
        P_1 = yf1 * P_local
        P_2 = yf2 * P_local
    ...
    # Pass 2: face velocities from Ergun balance
    u_face = np.empty(N + 1)
...
species_cmaps = [plt.cm.Greys, plt.cm.Blues, plt.cm.Reds]
